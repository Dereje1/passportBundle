...
import { processLogin, passportConfig } from '../../src/auth/passportConfig';
import user from '../user_stub';
...
describe('Processing a login', () => {
  ...
  test('will create a google user if not found in the db', async () => {
    await processLogin('anytoken', '', googleProfile as Profile, done, user);
  ...
  test('will create a twitter user if not found in the db', async () => {
    ...
    await processLogin('anytoken', '', twitterProfile as Profile, done, user);
    expect(user.findOne).toHaveBeenCalledTimes(1);
    expect(user.findOne).toHaveBeenCalledWith({
  ...
  test('will create a github user if not found in the db', async () => {
    ...
    await processLogin('anytoken', '', githubProfile as Profile, done, user);
    expect(user.findOne).toHaveBeenCalledTimes(1);
    expect(user.findOne).toHaveBeenCalledWith({
...
describe('Configuring passport', () => {
  ...
  test('will succeed for twitter', () => {
    ...
    passportConfig(passport as unknown as PassportStatic, user);
